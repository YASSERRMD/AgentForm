...
import { validateLimits, type ValidateLimitsOptions } from './limits.js';
...
/** Runs every semantic check (§4 Phase 4's minimum-checks list, plus §19's size/complexity limits) against a schema-valid document and returns every diagnostic found — never throws, never short-circuits on the first failing check. */
export function validateSemantics(
  application: AgenticApplication,
  limitsOptions: ValidateLimitsOptions = {},
): Diagnostic[] {
  return [
    ...
    ...validateLimits(application, limitsOptions),
  ];
}
