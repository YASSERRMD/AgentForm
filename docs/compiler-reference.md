...
## Agno adapter (`@agentform/adapter-agno`)

Generates a Python project (`src/{agents,tools,workflows}/`, `pyproject.toml` pinning `agno==2.8.0` and `fastapi==0.139.2`, `.env.example`, `README.md`) targeting the real `agno` package — added post-v1, not part of the original six-target release.

| Agentform concept                                                  | Generated as                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Agent                                                              | `build_<id>_agent() -> Agent`, a real `agno.agent.Agent`. `instructions` maps directly; `role`+`description` both map into Agno's own `description` field (Agno's `role` field means something different — "this agent's role _in a Team_", verified against the installed source — not used here)                                                                                                                |
| Model                                                              | `agno.models.<provider>.<Class>(id=...)` for `openai`/`anthropic`/`google` (verified constructing each against the installed package); any other provider gets `model=None` with a TODO citing `docs.agno.com/models`                                                                                                                                                                                             |
| Tool (any of the 9 IR tool types)                                  | An `@tool(...)`-decorated function; `requires_confirmation=True` for a `humanApproval`-type tool or any `sideEffect: destructive` tool — Agno's own real, blocking tool-level confirmation gate                                                                                                                                                                                                                   |
| Workflow                                                           | `build_<id>_workflow() -> Workflow`, a real `agno.workflow.Workflow(steps=[...])` — see below for how the general node/edge graph maps onto Agno's step-sequence model                                                                                                                                                                                                                                            |
| `agent`/`tool`/`terminate`/`transform`/`delay`/`subworkflow` nodes | Each its own `Step(...)` — `delay` calls a real `time.sleep()` (a concrete duration, not an expression); `subworkflow` nests another generated `Workflow` via `Step(workflow=...)`; `tool`/`transform` bodies are `NotImplementedError` stubs (Agno tools are designed to be invoked by an agent's own reasoning, not run as standalone steps; `transform`'s `expression` has no evaluator anywhere in Agentform) |
| `humanApproval` node                                               | `Step(requires_confirmation=True)` — a real, blocking Agno human-in-the-loop gate, not an emulation                                                                                                                                                                                                                                                                                                               |
| `loop`/`parallel`/`router`/`condition`                             | Real `Loop`/`Parallel`/`Router`/`Condition` constructs — `parallel`'s branches and `router`/`condition`'s choices come from the node's own declared `branches`/edges (unambiguous, not guessed); `loop` bodies and `router`/`condition` selector/evaluator logic are `NotImplementedError` stubs                                                                                                                  |

**Scope**: `agent`, `tool`, `humanApproval`, `loop`, `parallel`, `router`, `condition`, `subworkflow`, `transform`, `delay`, `terminate` are all `supported` — the richest node-type coverage of any adapter after LangGraph, since Agno's own workflow primitives map unusually directly onto Agentform's node vocabulary. Only `join` and `event` are `unsupported`: Agno's join semantics belong to the `Parallel` construct that fans out, not to a standalone downstream node; waiting on an external event trigger has no synchronous `Step` equivalent to translate to honestly. Agentform's workflow model is a general graph (cycles allowed only through `loop` nodes); Agno's is fundamentally an ordered step sequence with structured nesting — this adapter resolves the gap with a cycle-safe depth-first walk from the entrypoint (not a global topological sort, which was tried first and found to fail completely whenever the entrypoint itself sits inside a loop's cycle — every node ends up unplaceable, not just the loop's own back-edge) rather than attempting full graph-region reconstruction. **Agent delegation is `partial`**: this adapter generates plain `Agent` objects wired into `Workflow` `Step`s, not an `agno.team.Team` — Agno's real multi-agent delegation construct (`Team(members=[...], mode=TeamMode.coordinate)`, verified against the installed package) — binding `allowedAgents` to a Team's member set is future adapter work.

**Verified, not assumed**: every construct above — `Agent`, `@tool`, and every one of `Step`/`Loop`/`Parallel`/`Condition`/`Router`/`HumanReview`, including a full `Workflow` mixing all of them together — was actually constructed (not just signature-checked) against the real installed `agno==2.8.0` package in an isolated virtualenv before any code generation logic was written. A real generated project (all 11 supported node types in one workflow, including a genuine loop cycle and a nested subworkflow) was then generated by the real TypeScript codegen, written to disk, and imported and constructed end-to-end with the real Agno package — `build_main_workflow()` returned a real `Workflow` with all 10 top-level steps correctly typed. One real, non-obvious finding from that process: `agno.workflow`'s own `__init__.py` unconditionally imports `agno.workflow.remote.RemoteWorkflow`, which requires `fastapi` — even though `agno`'s own package metadata marks `fastapi` as belonging to an optional extra (`os`/`dev`/`demo`). This adapter pins `fastapi` as a direct dependency of every generated project rather than trusting that metadata.

## Cross-adapter compatibility matrix

Transcribed directly from each adapter's own `compatibility.ts` (`NODE_TYPE_LEVELS`/`SUPPORTED_NODE_TYPES`), not hand-guessed — a cross-target test (`apps/cli/src/cross-target.test.ts`) compiles a real spec against every real adapter and asserts zero blocking diagnostics for the row every column marks `supported`, so this table can't silently drift from the code.

| Workflow node type | openai      | langgraph   | microsoft   | google-adk  | autogen     | crewai      | agno        |
| ------------------ | ----------- | ----------- | ----------- | ----------- | ----------- | ----------- | ----------- |
| `agent`            | supported   | supported   | supported   | supported   | supported   | supported   | supported   |
| `terminate`        | supported   | supported   | supported   | supported   | supported   | supported   | supported   |
| `tool`             | supported   | supported   | unsupported | unsupported | unsupported | unsupported | supported   |
| `humanApproval`    | unsupported | supported   | unsupported | unsupported | emulated    | unsupported | supported   |
| `loop`             | unsupported | supported   | unsupported | unsupported | unsupported | unsupported | supported   |
| `router`           | unsupported | supported   | unsupported | unsupported | unsupported | unsupported | supported   |
| `parallel`         | unsupported | unsupported | unsupported | unsupported | unsupported | unsupported | supported   |
| `condition`        | unsupported | unsupported | unsupported | unsupported | unsupported | unsupported | supported   |
| `subworkflow`      | unsupported | unsupported | unsupported | unsupported | unsupported | unsupported | supported   |
| `transform`        | unsupported | unsupported | unsupported | unsupported | unsupported | unsupported | supported   |
| `delay`            | unsupported | unsupported | unsupported | unsupported | unsupported | unsupported | supported   |
| `join`/`event`     | unsupported | unsupported | unsupported | unsupported | unsupported | unsupported | unsupported |

Only `agent`/`terminate` are `supported` by every target — that's the portable baseline the cross-target test exercises. `tool` as its own _workflow node_ is an OpenAI/LangGraph/Agno-only concept: the other four targets all wire tools onto agents directly (`agent.tools`) rather than as a separate graph node, so a `tool`-type node has no representation there regardless of the underlying tool _type_ — every adapter can generate a stub for all nine IR tool types (`function`/`http`/`openapi`/`mcp`/`database`/`queue`/`agent`/`humanApproval`/`customPlugin`) identically; it's the node-graph placement that differs, not tool-generation capability. `join`/`event` remain unsupported everywhere — no adapter has a construct to bind either to.

**Agent delegation** (`agent.delegation.allowedAgents`) isn't a workflow node, so it doesn't fit the table above — and it's the dimension where the targets diverge the most, each shaped by a real, verified constraint of its underlying framework:
...
- **agno**: `partial` — this adapter generates plain `Agent`s wired into `Workflow` `Step`s, not Agno's own real `Team` delegation construct; see the Agno section above.
