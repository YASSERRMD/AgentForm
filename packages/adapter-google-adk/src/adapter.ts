...
export const GOOGLE_ADK_ADAPTER_MANIFEST: AgentformPluginManifest = {
  name: '@agentform/adapter-google-adk',
  version: '0.1.0',
  apiVersion: 'agentform.dev/v1alpha1',
  type: 'FrameworkAdapter',
  capabilities: ['agent-hierarchy', 'tools', 'sessions', 'model-configuration'],
  supportedSpecVersions: ['v1alpha1'],
};

/**
 * `@agentform/adapter-google-adk`'s `FrameworkAdapter` implementation. §13.4
 ...
 * Python layout (`src/{agents,tools,workflows}/`), `pyproject.toml`/
 * `.env.example`/`README.md`, plus `__init__.py` files in every package
 * directory (Python relative imports require it). No `models/` directory,
 * unlike `@agentform/adapter-autogen`: ADK's `model` field is a plain
...
export const googleAdkAdapter: FrameworkAdapter = {
  manifest: GOOGLE_ADK_ADAPTER_MANIFEST,

  async validateCompatibility(ir: AgentformIR): Promise<CompatibilityReport> {
    return validateGoogleAdkCompatibility(ir);
  },
  ...
  async generate(ir: AgentformIR, context: GenerationContext): Promise<GeneratedProject> {
    ...
    const manifest = buildManifest({
      adapter: GOOGLE_ADK_ADAPTER_MANIFEST,
    ...
    return { target: 'google-adk', files, manifest };
  },
};
