...
- **Source file size is bounded.** `loadDocument` checks a file's byte size against `maxSourceFileSizeBytes` (default 2 MiB) _before_ parsing it — an oversized file is rejected as `AGF1010` instead of being handed to the YAML parser at all.
