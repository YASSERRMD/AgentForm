...
| `spec.workflows`     | `workflowSchema` (map)                  | `entrypoint` and at least one entry in `nodes` are required. `nodes` is a discriminated union over the thirteen node types from §6.5. `edges` connect nodes with an optional `when` guard expression (validated as non-empty text only, and never evaluated — see Expressions below). |
...
Three fields hold expression text: an edge's `when`, a `condition` node's `condition`, and a `transform` node's `transform`. All three are validated as non-empty strings and nothing more — **no part of Agentform ever evaluates them.** There is no expression evaluator in the schema, the IR, the runtime, or any adapter.

What that means in practice:

- Under `agentform test`, the mock runtime picks a branch from the test case, not from the expression: a scenario declares `nodes["<id>"].next` for any node with more than one outgoing edge (`docs/evaluation-reference.md`). Semantic validation (`AGF3009`) already guarantees at most one outgoing edge is unconditional, so a genuine branch always requires the scenario to say which way it goes.
- Under `agentform compile`, the expression text is carried into the generated project as documentation next to a stub. Agno and LangGraph generate the branching node with a `raise NotImplementedError(...)` where the decision belongs; the other five adapters report `router`/`condition`/`transform` as `unsupported` and refuse to generate at all (`docs/compiler-reference.md`'s compatibility matrix).

Write `when:` expressions to state intent for a human reader and for the code you will complete yourself. Do not expect them to route anything on their own.
