...
export function baseIR(): AgentformIR {
  const result = buildIR({
    ...
    spec: {
      runtime: { target: 'crewai', environment: 'development' },
...
/**
 * A richer fixture exercising delegation, a tool, and a non-native-without-extras
 * model provider: two agents (one delegating to the other), a tool, and a
 * terminate node.
 */
export function multiAgentIR(): AgentformIR {
  const result = buildIR({
    ...
    spec: {
      runtime: { target: 'crewai', environment: 'production' },
      models: {
...
/** A `humanApproval`-type workflow node has no CrewAI generator (a real but task-scoped, review-and-refine primitive exists, with no faithful node-level binding) — used to exercise the "unsupported node type" branch of `validateCrewAiCompatibility`. */
