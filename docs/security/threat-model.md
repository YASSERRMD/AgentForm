...
As of Phase 9, Agentform validates, compiles to an IR, evaluates policy, can plan (compare desired state against a local SQLite-backed record of deployed state), and can compile a specification into a real project for any of its six target frameworks (OpenAI Agents SDK, LangGraph, Microsoft Agent Framework, Google ADK, AutoGen, CrewAI). It does not yet execute agents or actually apply state — several threats below are structurally about _runtime_ behavior Agentform doesn't have yet. Those are marked Planned rather than overclaimed as mitigated.
