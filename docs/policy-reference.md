...
| `AF014` | `state-must-not-contain-secrets`                 | error            | yes       | The deployed state a command read from its backend contains nothing credential-shaped. Passes cleanly when the caller supplied no state.    |
| `AF015` | `generated-code-must-be-reproducible`            | error            | yes       | **Not enforced here** — always passes; no generated artifact exists at policy-evaluation time. See Scope below.                             |
...
2. **A non-mandatory override that _weakens_ severity** (`error`→`warning`, `error`→`skip`, `warning`→`skip`) **requires a non-empty `justification`.** Without one, the override is rejected (`AGF4002`) and the default severity is kept.
...
- **`AF012` checks declaration, not enforcement.** There is no organization-level network allowlist registry to check a tool's `networkDestination` _against_ yet, and no HTTP-tool runtime to enforce it at request time — the policy only ensures the destination is stated explicitly rather than left unconstrained.
- **`AF014` only sees state a command already read.** `PolicyContext.state` is populated by `apply`, `status`, `drift`, and `rollback`, which have a state backend open; `validate`, `plan`, and `test` run before any state is read and leave it `undefined`, where the policy reports a clean pass rather than a false one. Most of `@agentform/state`'s shape cannot hold a secret by construction (`ResourceState` stores hashes, not values), so the check is defense in depth over the genuinely free-form parts — `deploymentIdentifiers` and apply-history summaries.
- **`AF015` is not enforced at policy-evaluation time**, and this is deliberate rather than a missing subsystem. The policy engine runs over the source document; generated code exists only in `agentform compile` (and `apply`'s generation step), and asserting reproducibility means generating twice and diffing — a whole-project operation at a stage where no other policy runs. What enforces the property today is the IR content hash that generation is a pure function of, plus a per-adapter "two `generate()` calls produce identical output" test in every adapter package. See ADR-0023.
- **No runtime enforcement of anything** — every check here is structural, over the source document. Whether a `function` tool flagged by `AF002` actually _runs_ a destructive shell command, whether a `humanApproval` gate is actually honored at execution time — none of that exists to check yet; it's adapter/runtime-phase scope (Phase 8+).
...
- **`AF014` always shows `pass` under `agentform validate`**: expected — `validate` has no state backend open, so there is no deployed state to inspect. Run `agentform status` or `agentform apply` against an applied project to exercise it for real.
- **`AF015` always shows `pass` no matter what I do**: expected, permanently — see Scope above. It is registered and mandatory so it cannot be disabled by an override config, but the property it names is enforced by adapter tests rather than by this check.
