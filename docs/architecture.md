...
## What the pipeline does not do: evaluate expressions

An architectural boundary worth stating before you write your first branching workflow: **Agentform has no expression evaluator at any stage.** An edge's `when`, a `condition` node's `condition`, and a `transform` node's `transform` are parsed, schema-validated, hashed into the IR, and emitted into generated code — and never executed by anything Agentform owns.

The specification pipeline is deliberately a _structural_ one. It knows a node has two outgoing edges and that one of them is guarded; it has no opinion about which guard is true for a given input, because deciding that would mean either embedding an expression language (a real language-design commitment, with its own sandboxing and versioning surface) or calling `eval` on spec text (never). Neither belongs in a control layer whose value proposition is determinism.

So the decision lives at the two edges of the pipeline instead. In testing, the scenario declares it (`nodes["<id>"].next`). In generated code, you implement it: the two adapters that support branching nodes (Agno, LangGraph) emit the node and its declared condition text around a `NotImplementedError` stub, and the other five report those node types `unsupported` rather than fabricating a translation. See `docs/schema-reference.md` (Expressions), `docs/evaluation-reference.md`, and `docs/compiler-reference.md`.
